...
class EnChTranslationDataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        en_tokens = ['BOS'] + en_tokenize(self.en[item]) + ['EOS']
        ...
        ch_tokens = ['BOS'] + ch_tokenize(self.ch[item]) + ['EOS']
        ch_valid_len = torch.tensor(len(ch_tokens), dtype=torch.long)
        ch_tokens_idx = torch.tensor(truncate_or_pad(ch_tokens, self.ch_vocab, self.num_steps+1), dtype=torch.long)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, encoder_output, dec_x, enc_valid_len, state):
        x = self.embedding(dec_x) * math.sqrt(self.hidden_dim) + self.pos_encoding[:, :dec_x.shape[1], :].to(dec_x.device)
        x = self.dropout(x)
        memory_mask = get_enc_mask(enc_valid_len, self.head_num, dec_x.shape[0], dec_x.shape[1], encoder_output.shape[1])
...
def get_enc_mask(enc_valid_len, head_num, batch_size, query_num, key_num):
    ...
    enc_mask = torch.arange(1, key_num + 1).reshape(1, -1).repeat(batch_size*head_num, query_num, 1).to(enc_valid_len.device) \
...
def get_dec_mask(head_num, batch_size, num_steps, training):
    if training:
        dec_mask = torch.triu(torch.ones(num_steps, num_steps), diagonal=1).type(torch.bool).repeat(batch_size*head_num, 1, 1)
    else:
        dec_mask = None
    return dec_mask
...
def train_transformer(train_iter, net, model_dim, batch_num, en_vocab, ch_vocab, devices, resume):
    ...
    while batch_id < batch_num and keep_training:
        for en_tokens_idx, en_valid_len, ch_tokens_idx, ch_valid_len in train_iter:
            ...
            dec_x = ch_tokens_idx[:, :-1].to(devices[0])
            true_y = ch_tokens_idx[:, 1:].to(devices[0])
            enc_x, enc_valid_len = en_tokens_idx.to(devices[0]), en_valid_len.to(devices[0])
            dec_valid_len = (ch_valid_len-1).to(devices[0])
            optimizer.zero_grad()
            y_hat, _ = net(enc_x, dec_x, enc_valid_len)
            l = MaskedCrossEntropyLoss(y_hat, true_y, dec_valid_len)
            l.backward()
            optimizer.step()
            train_loss.append(l.detach().cpu())
            pred = torch.argmax(y_hat, dim=2).squeeze()
            # bleu_list.append(calculate_bleu(pred, true_y, ch_vocab))
            if batch_id % 100 == 0:
                show_examples(enc_x, pred, true_y, en_vocab, ch_vocab, batch_id)
                print(f'train_loss: {train_loss[-1]}')
            del enc_x, dec_x, dec_valid_len, enc_valid_len, y_hat, l, pred, true_y
...
def predict(en_vocab, ch_vocab, num_steps, net, devices):
    while True:
        ...
        if en != 'stop':
            ...
            en_tokens_idx = torch.unsqueeze(torch.tensor(truncate_or_pad(en_tokens, en_vocab, num_steps), dtype=torch.long), dim=0).to(devices[0])
